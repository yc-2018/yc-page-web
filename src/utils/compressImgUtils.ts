 ...
 */
export const compressImageTo5MB = async (file: File) => {
  // 1. 检查是否非图片类型
  ...
  if (file.size <= MAX_SIZE) {
    console.log('文件小于5MB，直接返回:', file.name, (file.size / 1024 / 1024).toFixed(2) + 'MB');
    return file;
  }
  ...
  try {
    ...
    if (compressedFile.size > MAX_SIZE) {
      console.warn('压缩后文件仍大于5MB:', (compressedFile.size / 1024 / 1024).toFixed(2) + 'MB');
