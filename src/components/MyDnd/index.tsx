...
interface MyDndProps {
  dndIds: IBaseEntity[] | string[] | number[];        // 【必须】对象可拖动对象的ID数组  数组里面是 整个对象也行 但是这个对象必须包涵id属性
  setItems: Dispatch<SetStateAction<any[]>>;          // 【必须】设置items的函数 对象里面必须包含 id 属性***变不了的不知道为什么，但dragEndFunc可以
  storageName?: string;                               // 存储在localStorage中的名字  如果是空的就不存储
  style?: CSSProperties;                              // 最外层样式（有默认值）
  dragEndFunc?: (data: any[]) => void;                // 拖拽结束时触发的额外回调函数 而且会把拖动后的数组传进去
  ...
  return (
    <div style={style || {
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext
          items={dndIds.map((item: any) => item.id ?? item)}
